...
from monai.transforms import (EnsureChannelFirst,Activations,AddChannel,AsDiscrete,Compose,LoadImage,RandFlip,RandRotate,RandZoom,ScaleIntensity,EnsureType,Resize,RandRotate90)
...
model_pth = f'../Results/Medical_images/model_ResNet50_{max_epochs}epochs'
...
train_loader = DataLoader(train_ds, batch_size=30,num_workers=4, shuffle=True,pin_memory=torch.cuda.is_available())

val_ds = ImageDataset(image_files=val_x, labels=val_y, transform = val_transforms)
val_loader = DataLoader(val_ds, batch_size=30,num_workers=4,pin_memory=torch.cuda.is_available())
